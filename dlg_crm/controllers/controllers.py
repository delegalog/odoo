...
class OpportunityController(http.Controller):
    ...
    def get_opportunity(self, **kw):
        try:
            opportunity = http.request.env['dlg.crm.opportunity'].sudo().search_read([], ['id', 'name', 'customer',
                                                                                          'notes', 'done', 'image',
                                                                                          'phase_id', 'done'])
...
class PhaseController(http.Controller):
    ...
    def get_phase(self, **kw):
        try:
            phase = http.request.env['dlg.crm.phase'].sudo().search_read([], ['id', 'name', 'opportunity_id'])
