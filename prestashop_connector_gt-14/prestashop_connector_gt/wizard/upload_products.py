...
class upload_products(models.TransientModel):
    ...
    def upload_products(self, res_ids=None, presta_id=None):
        ...
        for rec in rec_list:
            ...
            prdct_search_ids = product_product_obj.search(
                [('product_tmpl_id', '=', rec)])  # CHECK IF THE PRODUCT HAS ANY VARIANTS
            if len(prdct_search_ids) > 1:
                ...
                for prd_id in prdct_search_ids:
                    ...
                    product_comb_vals = self.upload_combinations(prestashop, id_product, qty, product_option_id, ref,
                                                                 price)  # iF VARIANTS, UPLOAD THE NAME AND QUNATITY OF THAT VARIANT
                    product_comb_list.append(product_comb_vals)
                    self.upload_inventory(prestashop, product_obj, id_product, prd_id.id, variant=True,
                                          product_comb_vals=product_comb_vals)  # UPDATE THE INVENTORY

                product_schema['product']['associations']['combinations'].update(
                    {'combination': product_comb_list})  # ATTACH  THE COMBINATION TO PRODUCT
                product_schema['product'].update({'id_default_combination': product_comb_list[0]['id']})

            self.upload_inventory(prestashop, product_obj,
                                  id_product)  # To set the quantity of product we need to edit stock_availables.
            ...
            if product_obj.image_medium:
                for img_id in img_list:
                    ...
                    r = requests.delete(url + '/api/images/products/%s/%s' % (str(id_product), str(img_id)),
                                        files=files, auth=(key, ''))
    ...
    def upload_inventory(self, prestashop, product_obj, id_product, product_obj_id=None, variant=False,
                         product_comb_vals=None):
        prod_templ_obj = self.env['product.template']
        prdct_obj = self.env['product.product']
        inventry_line_obj = self.env['stock.inventory.line']

        if variant != True:
            stock_avail_id = prestashop.search('stock_availables', options={
                'filter[id_product]': id_product})  # search the stock for this product
            tmpl_ids = prod_templ_obj.search(
                [('default_code', '=', product_obj.default_code)])  # To search the product qty in invemtory
            ...
            if inv_id:
                ...
                stock_avail_schema = prestashop.get('stock_availables',
                                                    options={'schema': 'blank'})  # get blank dictionary
            ...
            stock_avail_id = prestashop.search('stock_availables', options={'filter[id_product]': id_product,
                                                                            'filter[id_product_attribute]':
                                                                                product_comb_vals['id']})
    ...
    def upload_product_options(self, prestashop, attr_id_list, attr_val_list):
        ...
        for atrr_id in attr_id_list:
            ...
            if not product_optn_search_ids:
                ...
                product_option_schema['product_option'].update(
                    {'public_name': {'language': {'attrs': {'id': '1'}, 'value': attr_name}},
            ...
            if attr_val_list:
                for vals in attr_val_list:
                    vals_id = self.env['product.attribute.value'].search(
                        [('name', '=', vals), ('attribute_id', '=', attr_id)])
                    if vals_id:
    ...
    def upload_product_option_value(self, prestashop, product_option_id, vals):
        ...
        if not product_vals_search_ids:
            ...
            product_vals_schema['product_option_value'].update({'id_attribute_group': product_option_id,
                                                                'name': {
                                                                    'language': {'attrs': {'id': '1'}, 'value': vals}},
                                                                })
    ...
    def upload_combinations(self, prestashop, product_id, quanty, product_vals_ids, ref, price):
        ...
        if not product_comb_search_ids:
            ...
            product_comb_schema['combination']['associations']['product_option_values'].update(
                {'product_option_value': product_vals_list})
    ...
    def upload_images(self, presta_inst_id, id_product, image_data):
        ...
        #         image_id  = False
        files = {'image': data}
        r = requests.post(url + '/api/images/products/' + str(id_product), files=files, auth=(key, ''),
